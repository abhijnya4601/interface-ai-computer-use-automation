...
The image's default command (`webconsole/serve.sh`) runs the mock bank (`:5050`, internal), the
hostile-DOM target with the `?inject=` fault switch (`:5051`, internal), and the console on
`$PORT` (public). Discovered capabilities go to `$CAPABILITIES_DIR` and run history to
`$CONSOLE_RUNS_PATH` - point both at a volume to persist them.
